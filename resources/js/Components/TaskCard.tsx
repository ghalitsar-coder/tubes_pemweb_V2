...
const TaskCard: React.FC<TaskCardProps> = ({ task }) => {
  ...
  return (
    <div className={`task-card bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg ${task.is_template ? 'border-2 border-purple-200 dark:border-purple-900' : ''}`}>
      <div className="p-5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex-shrink-0">
            <span className={`status-badge inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
              task.is_template 
                ? 'bg-purple-100 text-purple-800 dark:bg-purple-900 dark:text-purple-300' 
                : getStatusBadgeClass(task.status)
            }`}>
              {task.is_template ? 'Template' : task.status.replace('-', ' ')}
            </span>
          </div>
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
            <div 
              className={`progress-bar ${getProgressBarColor(task.status, task.is_template)} h-1.5 rounded-full`} 
      ...
      <div className="bg-gray-50 dark:bg-gray-900 px-5 py-3 flex justify-end">
        <div className="flex space-x-3">
          ...
          <Link href={route('tasks.edit', task.id)} className="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300">
